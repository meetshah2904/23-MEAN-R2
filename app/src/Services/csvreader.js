...
function readfile() {
    let myFile = fs.createReadStream("D:\\MEAN23\\23-MEAN-2\\app\\src\\Seed\\ind_nifty50list.csv", "utf8") // fopen("data.txt","r") 
    myFile
...
module.exports.uploadIndustry = async function () {
    let industryArray = []
    let myFile = fs.createReadStream("D:\\MEAN 2023\\23-MEAN-2\\app\\src\\Seed\\ind_nifty50list.csv", "utf8") // fopen("data.txt","r") 
    let myDataFromDb = []
...
module.exports.uploadEquity = async function () {
    let eqArray = [] 
    let myFile = fs.createReadStream("D:\\MEAN 2023\\23-MEAN-2\\app\\src\\Seed\\ind_nifty50list.csv", "utf8")
    let industryDb = []
    let equityDb = [] 
    IndustryModel.find().then(data => {
        industryDb = data;
